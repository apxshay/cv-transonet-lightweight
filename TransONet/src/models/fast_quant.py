...
def percentile_search(x, w, bias, z0, bit, search_space=200):#, best_act_scale = None):
    ...
    #print('best_act_scale: {}'.format(best_act_scale))
...
def quant_forward(self, x: Tensor) -> Tensor:
    ...
    if self.clip_search:
        ...
        #i +=1
        #print('i {}'.format(i))
        #self.best_act_scale = best_act_scale
        self.act_scale = percentile_search(x, self.weight, self.bias, z0, self.bit, search_space=1000)#, best_act_scale = best_act_scale)
        # if self.act_scale is not None:
        #     self.best_act_scale = self.act_scale
...
def fast_quant(model, bit=6, with_noisy_quant=False, percentile=False, search_mean=False, search_noisy=False):
    ...
    for name, module in model.named_modules():
        ...
        #print('module name: {}'.format(module.own_name))
        #module.best_act_scale = best_act_scale
        if isinstance(module, nn.Linear) and name != "head":
            ...
            # if module.best_act_scale is not None:
            #     best_act_scale = module.best_act_scale
